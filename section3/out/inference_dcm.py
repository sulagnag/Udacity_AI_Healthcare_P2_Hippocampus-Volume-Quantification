...
def create_report(inference, header, orig_vol, pred_vol):
    ...
    norig_vol = med_reshape(volume, new_shape=(volume.shape[0], 64, 64))
    for i,sl in enumerate(slice_nums):
        nd_img = np.flip((norig_vol[sl,:,:]/np.max(norig_vol[sl,:,:]))*0xff).T.astype(np.uint8)
    # This is how you create a PIL image from numpy array
        pil_i = Image.fromarray(nd_img, mode="L").convert("RGBA").rotate(90)
       
        pred_img = np.flip((pred_vol[sl,:,:]/np.max(pred_vol[sl,:,:]))*0xff).T.astype(np.uint8)
    # This is how you create a PIL image from numpy array
        pil_ip = Image.fromarray(pred_img, mode="L").convert("RGBA").rotate(90)
        r,b,g,a=pil_ip.split() 
    ...
    # Paste the PIL image into our main report image object (pimg)
        ...
        re =Image.blend(pil_i, result, 0.5).resize((400,400))
       
        if i==2:
            pimg.paste(re,box=(100,480))    
        else:
            pimg.paste(re,box=(100+400*i,80))
 
    draw.text((10, 0), "HippoVolume.AI", (255, 255, 255), font=header_font)
    draw.multiline_text((10, 90),
                         f"Patient ID: {header.PatientID}\n"
                         f"Patient's Name: {header.PatientName}\n"
                         f"Study Date: {header.StudyDate}\n"
                         f"Modality : {header.Modality}\n"
                         f"Study Description : {header.StudyDescription}\n"
                         f"Volume Anterior : {inference['anterior']}\n"
                         f"Volume Posterior : {inference['posterior']}\n"
...
def get_series_for_inference(path):
    ...
    series_for_inference=[d for d in dicoms if d.Modality =="MR" ]
...
if __name__ == "__main__":
    ...
    os_command("storescu localhost 4242 /home/workspace/src/report.dcm")
